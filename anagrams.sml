...
fun lookup Empty (x,a) =false
  | lookup (Node(_,(y,b),left,right)) (x,a) =
	...
	fun pick ((nil,NONE,l2,seq,premove),que,already)=(*let val _ =print("problem")in*)(que,already) (*end*) 
	...
	val already=add1(Empty,(implode(xs),"x"))
	
	val state =(xs,SOME x,nil,["10"],"10")
	val _=Queue.enqueue(que,state)
	val already=add1(already,(implode(xs),""))
